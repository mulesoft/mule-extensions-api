...
/**
 * Callback used by {@link Source sources} in order tell the runtime that they will process the flow's response
 ...
 * <li>Response operation is blocking, but it's asynchronously executed in a thread other than the one which initiates it
 * (most typically, the thread executing a method annotated with {@link OnSuccess} or{@link OnError}
 * </li>
 ...
 * </ul>
 * <p>
 * In those use cases, the runtime needs to know that the response will be asynchronous because otherwise it will
 * generate a race condition between the thread emitting the response and the runtime itself which is trying to free
 ...
 *
 * @since 1.0
 * @deprecated use {@link org.mule.sdk.api.extension.runtime.source.SourceCompletionCallback} instead.
 */
@NoImplement
@Deprecated
