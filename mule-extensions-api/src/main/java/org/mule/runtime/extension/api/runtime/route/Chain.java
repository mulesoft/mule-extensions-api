...
/**
 * A {@link Chain} allows a given {@link OperationModel Operation} to receive a chain of message processors
 * to be executed as part of the operation's execution.
 * <p>
 * When executing the {@link Chain}, all it's inner components will be executed in the order declared by the
 * user, using the output of one processor as the input of the next one in the {@link Chain}. The execution
 * of the {@link Chain} is completed either when all the components were invoked successfully or when an error
...
public interface Chain {
  ...
  /**
   * Executes the chain of components starting with the given {@link Result} data as input {@link Message}.
   * This method should be used whenever the input of this {@link Chain} is the output of another component execution.
   * <p>
   * {@code onSuccess} callback will be invoked with the output {@link Result} of the last component
   * in the {@link Chain} if no error occurred after all the components were executed exactly once.
   * <p>
