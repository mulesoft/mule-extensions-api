...
public interface SourceModelParser extends SemanticTermsParser, StereotypeModelParser, AdditionalPropertiesModelParser,
  ...
  /**
   * Returns the {@link SdkSourceFactory} that brings the source to life.
   *
   * @return a {@link SdkSourceFactory}
   */
  Optional<SdkSourceFactory> getSourceFactory();
  ...
  /**
   * @return a {@link MediaTypeParser} describing the source's output mimeType, if one was defined
   */
  Optional<MediaTypeParser> getMediaType();

  /**
   * @return an {@link Optional} {@link SdkExceptionHandlerFactory} if an exception handler was defined for this source.
   */
