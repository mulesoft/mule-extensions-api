...
import org.mule.runtime.extension.api.test.dsl.model.ChildOfAbstractType;
...
import org.mule.runtime.extension.api.test.dsl.model.InterfaceDeclarationWithMapping;
import org.mule.runtime.extension.api.test.dsl.model.InterfaceImplementation;
import org.mule.runtime.extension.api.test.dsl.model.NonDefaultConstructor;
...
public class ParameterXmlDeclarationTestCase extends BaseXmlDeclarationTestCase {
  ...
  public void testComplexPojoParameter() {
    ...
    assertElementPrefix(PREFIX, result);
    assertParameterChildElementDeclaration(true, result);
    assertIsWrappedElement(false, result);
  ...
  public void xmlStyleAtTypeLevel() {
    ...
    when(parameterModel.getType())
        .thenReturn(TYPE_BUILDER.stringType().with(new TypeDslAnnotation(false, false, "", "")).build());
    when(parameterModel.getDslConfiguration()).thenReturn(dslModel);
  ...
  public void testExtensibleImportedTypeWithoutXmlParameter() {
    ...
    assertElementPrefix(IMPORT_PREFIX, result);
    assertParameterChildElementDeclaration(true, result);
    assertIsWrappedElement(true, result);
  }
  ...
  public void testMapOfSimpleTypeParameter() {
    MetadataType valueType = TYPE_BUILDER.stringType().build();
    when(parameterModel.getType()).thenReturn(TYPE_BUILDER.objectType()
        .with(new ClassInformationAnnotation(Map.class))
        .openWith(TYPE_BUILDER.stringType())
        .build());
  ...
  public void testCollectionOfSimpleTypeParameter() {
    MetadataType itemType = TYPE_BUILDER.stringType().build();
    when(parameterModel.getName()).thenReturn(SINGULARIZABLE_PARAMETER_NAME);
    when(parameterModel.getType()).thenReturn(TYPE_BUILDER.arrayType()
        .of(itemType)
        .build());
  ...
  public void testCollectionOfComplexTypeParameter() {
    ...
    ifContentParameter(() -> {
      ...
      assertElementPrefix(PREFIX, innerElement);
      assertParameterChildElementDeclaration(true, innerElement);
    });
  ...
  public void testMapOfListOfComplexTypeParameter() {
    MetadataType itemType = TYPE_LOADER.load(ExtensibleType.class);
    MetadataType valueType = TYPE_BUILDER.arrayType().of(itemType).build();
    ...
    assertElementPrefix(PREFIX, mapDsl);
  ...
  public void testListOfMapsParameter() {
    ObjectType dictionary = TYPE_BUILDER.objectType()
        .with(new ClassInformationAnnotation(Map.class))
        .openWith(TYPE_BUILDER.stringType())
        .build();

    when(parameterModel.getName()).thenReturn(COLLECTION_NAME);
    when(parameterModel.getType()).thenReturn(TYPE_BUILDER.arrayType().of(dictionary).build());
    DslElementSyntax result = getSyntaxResolver().resolve(parameterModel);
    ifNotContentParameter(() -> {
      assertAttributeName(COLLECTION_NAME, result);
      assertElementName(hyphenize(COLLECTION_NAME), result);
